...
import type { AccountRow } from '@/entities/dashboard/model/types'
...
const COLUMNS: DataTableColumn<AccountRow>[] = [
  ...
  { key: 'DATE_END', header: 'Дата выдачи' },
  {
    ...
    header: 'Клиент',
    wrap: true,
    render: (r) => cleanName(r.OWNER_NAME) || '—',
  },
  {
    key: 'MARK_NAME',
    header: 'Марка / модель',
    render: (r) =>
      [cleanName(r.MARK_NAME), cleanName(r.MODEL_NAME)].filter(Boolean).join(' / ') || '—',
  },
  { key: 'MASTER_NAME', header: 'Мастер', render: (r) => formatShortName(r.MASTER_NAME) || '—' },
  {
    key: 'MAIN_MECHANIC_NAME',
    header: 'Механик',
    render: (r) => formatShortName(r.MAIN_MECHANIC_NAME) || '—',
  },
  { key: 'SUMMA', header: 'Сумма', render: (r) => formatCurrency(r.SUMMA) },
  { key: 'LABOR_TIME', header: 'Нормо-часы', render: (r) => formatHours(r.LABOR_TIME) },
  {
    key: 'SERVICE_TIME_TO_CALC',
    header: 'Время ремонта',
    render: (r) => formatServiceTime(r.SERVICE_TIME_TO_CALC),
  },
  ...
  { key: 'T_FACTOR', header: 'Т-фактор', render: (r) => formatPercent(r.T_FACTOR) },
  {
    ...
    render: (r) =>
      r.CARELESS_BILL ? (
        <span
          title="Заказ-наряд закрыт неаккуратно"
          style={{ color: 'var(--color-negative)', fontWeight: 700 }}
        >
          !
        </span>
      ) : (
