...
export function BarChart<T extends Record<string, unknown>>({
  ...
  const hasHighlight =
    highlightKey !== undefined && highlightValue !== undefined && highlightValue !== null
  ...
  return (
    <ResponsiveContainer width="100%" height={resolvedHeight}>
      <RBarChart data={data} layout={layout} margin={{ top: 8, right: 16, left: 0, bottom: 8 }}>
          ...
          {hasHighlight &&
            data.map((item, i) => (
              <Cell
                key={i}
                fill={
                  item[highlightKey as string] === highlightValue
                    ? CHART_COLORS.accent
                    : CHART_COLORS.border
                }
              />
          ...
          {!hasHighlight &&
            ...
            data.map((item, i) => (
              <Cell
                key={i}
                fill={
                  ((item[dataKey] as number) || 0) < 0 ? CHART_COLORS.negative : CHART_COLORS.accent
                }
              />
            ))}
        </Bar>
