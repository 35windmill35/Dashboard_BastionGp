...
type RegisterStep = 'phone' | 'code' | 'password' | 'done'

function getAttemptsLeftFromError(err: unknown): number | null | undefined {
  if (!(err instanceof ApiError)) return undefined
  const data = err.data as { AttemptsLeft?: number } | undefined
...
export function useRegister() {
  const [step, setStep] = useState<RegisterStep>('phone') // 'phone' | 'code' | 'password' | 'done'
    ...
    async (code: string): Promise<boolean> => {
      ...
      try {
        ...
        if (!didLogin) {
          setError(
            authStore.error ||
              'Код подтверждён, но не удалось завершить регистрацию. Попробуйте ещё раз.'
          )
          return false
        }
        ...
        const freshAttemptsLeft = getAttemptsLeftFromError(err)
        ...
        setError(
          getErrorMessage(err, {
            ...
            statusMessages: {
              ...
              27:
                'Неверный код' +
                (freshAttemptsLeft != null ? `, осталось попыток: ${freshAttemptsLeft}` : ''),
            },
          })
        )
        return false
