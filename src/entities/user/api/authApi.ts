...
interface RegisterByPhoneArgs {
  phone: string
  dbGuid: string | null
}

export interface RegisterByPhoneResult {
  TimeLeft?: number
  AttemptsLeft?: number
  [key: string]: unknown
}
...
export async function registerByPhone({
  phone,
  dbGuid,
}: RegisterByPhoneArgs): Promise<RegisterByPhoneResult> {
  const { data } = await getWithBasicAuth<RegisterByPhoneResult>('/api-v2/auth/registerByPhone', {
    username: phone,
    password: 'Register!',
...
export async function confirmCode({
  ...
  code,
  dbGuid,
}: ConfirmCodeArgs): Promise<ConfirmCodeResult> {
  const { data } = await getWithBasicAuth<ConfirmCodeResult>('/api-v2/auth/confirmCode', {
    username: phone,
    password: code,
    params: { AppCode: APP_CODE },
    headers: dbGuid ? { Params: JSON.stringify({ DB_GUID: dbGuid }) } : undefined,
