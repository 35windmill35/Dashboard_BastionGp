...
interface ScatterChartProps<T extends Record<string, unknown>> {
  data: T[]
  xKey: string
  yKey: string
  nameKey?: string
              ...
              >
                {nameKey && (
                  <div style={{ fontWeight: 600 }}>
                    {nameFormatter ? nameFormatter(point[nameKey]) : (point[nameKey] as ReactNode)}
                  </div>
                )}
                <div>
                  {xLabel}: {point[xKey] as ReactNode}
                </div>
                <div>
                  {yLabel}: {point[yKey] as ReactNode}
                </div>
              </div>
            )
          }}
