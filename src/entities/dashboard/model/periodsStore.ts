...
import type { PeriodRow, PeriodOption, Period } from './types'

class PeriodsStore {
  periods: PeriodRow[] = []
  selectedPeriodYm: number | null = null
  ...
  periodMode: PeriodMode = 'month' // 'month' | 'quarter' | 'year'
  selectedYear: number | null = null
  selectedYq: number | null = null
  ...
  constructor() {
    ...
    reaction(
      () => [authStore.isAuthenticated, authStore.dbIndex] as const,
  ...
  get periodOptions(): PeriodOption[] {
    return this.monthPeriods.map((p) => ({
      value: p.PERIOD_YM as number,
      label: formatPeriodLabel(p.PERIOD_YM as number),
    }))
  }
  ...
  get yearOptions(): PeriodOption[] {
    return this.periods
      .filter((p) => p.PERIOD_TYPE === 'Y')
      .map((p) => ({
        value: p.PERIOD_YEAR as number,
        label: formatYearLabel(p.PERIOD_YEAR as number),
      }))
      .sort((a, b) => b.value - a.value)
  }

  get quarterOptions(): PeriodOption[] {
    return this.periods
      .filter((p) => p.PERIOD_TYPE === 'Q')
      .map((p) => ({
        value: p.PERIOD_YQ as number,
        label: formatQuarterLabel(p.PERIOD_YQ as number),
      }))
      .sort((a, b) => b.value - a.value)
  }
  ...
  get selectedPeriod(): Period | null {
    if (this.periodMode === 'year')
      return this.selectedYear ? { type: 'year', value: this.selectedYear } : null
    if (this.periodMode === 'quarter')
      return this.selectedYq ? { type: 'quarter', value: this.selectedYq } : null
  ...
  async load(dbIndex: number): Promise<void> {
    this.isLoading = true
    this.error = null
    ...
    try {
      ...
      runInAction(() => {
        ...
        this.selectedYear = periods.length ? periodYmToYear(this.selectedPeriodYm as number) : null
        this.selectedYq = periods.length ? periodYmToYq(this.selectedPeriodYm as number) : null
  ...
  pickDefaultPeriod(periods: PeriodRow[]): number | null {
    const months = periods.filter((p) => p.PERIOD_TYPE === 'M')
    if (!months.length) return null
    return (months[1]?.PERIOD_YM ?? months[0].PERIOD_YM) as number
  }

  setSelectedPeriod(periodYm: number): void {
  ...
  setSelectedQuarter(yq: number): void {
    this.selectedYq = yq
  }
  ...
  // данных.
  get previousPeriodYm(): number | null {
    ...
    return (months[index + 1]?.PERIOD_YM ?? null) as number | null
  }
}
